...
class PageE2EVerifier:
    def verify(
        ...
        *,
        ...
        dependencies = (
            plan.get("dependencies") if isinstance(plan.get("dependencies"), list) else []
        )
        ...
        diagnostics: list[str] = []
        checks.append(
            {"name": "planned_dependency_count", "passed": True, "count": len(planned_dependencies)}
        )
        ...
        dependencies_meta = (
            meta.get("dependencies") if isinstance(meta.get("dependencies"), dict) else {}
        )
        function_bindings = (
        ...
        for item in planned_dependencies:
            ...
            if not binding_exists:
                diagnostics.append(
                    f"依赖 {key} 未正确写入页面 bindings（function_id={function_id}）"
                )
