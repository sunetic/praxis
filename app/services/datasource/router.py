...
def resolve_datasource_by_role(
    ...
    current = (
        db.query(models.DataSource).filter(models.DataSource.id == current_datasource_id).first()
    )
    if not current:
    ...
    if target_role == "sys":
        sys_candidates = [
            item for item in siblings if normalize_role(item.tenant_role or "user") == "sys"
        ]
        if not sys_candidates:
            raise DataSourceRoutingError(f"No sys datasource found in cluster '{cluster_key}'.")
    ...
    if not user_candidates:
        raise DataSourceRoutingError(f"No user datasource found in cluster '{cluster_key}'.")
