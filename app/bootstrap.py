...
async def _handle_stats_analysis_schedule(runtime_service, schedule, *, payload, trigger_type, trace_id):
    ...
    if not isinstance(datasource_id, int):
        datasource_id = None if mode == "batch" else (schedule.target_id if isinstance(schedule.target_id, int) else None)
    ...
    try:
        if isinstance(datasource_id, int):
            result = await run_stats_risk_collect_for_datasource(
                datasource_id, lookback_days=lookback_days, stale_days=stale_days, trigger_type=trigger_type_str,
            )
            return ScheduleRuntimeResult(
                run_id=trace_id or "", status="success",
                output=result.model_dump(), output_summary=None,
        ...
        for ds_id in datasource_ids:
            try:
                item_result = await run_stats_risk_collect_for_datasource(
                    ds_id, lookback_days=lookback_days, stale_days=stale_days, trigger_type=trigger_type_str,
                )
                ...
                failures += 1
                item = {"datasource_id": ds_id, "status": "failed", "error_summary": str(exc),
                        "collected_tables": 0, "active_candidates": 0, "expired_candidates": 0}
            items.append(item)

        status = "success" if failures == 0 else ("success" if failures < len(datasource_ids) else "failed")
        return ScheduleRuntimeResult(
            run_id=trace_id or "", status=status,
            output={"mode": "batch", "datasource_count": len(datasource_ids),
                    "succeeded": len(datasource_ids) - failures, "failed": failures,
                    "collected_tables": total_collected, "active_candidates": total_active,
                    "expired_candidates": total_expired, "items": items},
            output_summary=None,
            error_class=None if status == "success" else "runtime",
            error_message=None if status == "success" else "stats_analysis batch collect failed for all datasources",
            duration_ms=max(int((datetime.utcnow() - started_at).total_seconds() * 1000), 0),
        )
    except Exception as exc:
        return ScheduleRuntimeResult(
            run_id=trace_id or "", status="failed", output=None, output_summary=None,
...
async def _handle_collector_schedule(runtime_service, schedule, *, payload, trigger_type, trace_id):
    ...
    try:
        collector_runtime = CollectorRuntimeService(session_factory=runtime_service._session_factory)
        payload.update({"schedule_id": schedule.id, "trigger_type": trigger_type, "trace_id": trace_id})
