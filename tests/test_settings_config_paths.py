import pytest
from pydantic import ValidationError

from app.core.config import PROJECT_ROOT, Settings, _normalize_sqlite_url, _resolve_project_path
from app.db.database import _ensure_sqlite_parent_directory


def test_normalize_sqlite_url_resolves_relative_path_from_project_root():
    normalized = _normalize_sqlite_url("sqlite:///./praxis.db")

    assert normalized == f"sqlite:///{(PROJECT_ROOT / 'praxis.db').resolve()}"


def test_resolve_project_path_keeps_absolute_path():
    absolute = PROJECT_ROOT / "custom.db"

    assert _resolve_project_path(str(absolute)) == absolute.resolve()


def test_retired_agent_settings_are_rejected():
    with pytest.raises(ValidationError, match="Extra inputs are not permitted"):
        Settings(_env_file=None, agent_soft_finalize_seconds=240)
    assert not any(name.startswith("agent_") for name in Settings.model_fields)


def test_sqlite_parent_directory_is_created_for_clean_install(tmp_path):
    database_path = tmp_path / "missing" / "nested" / "praxis.db"

    _ensure_sqlite_parent_directory(f"sqlite:///{database_path}")

    assert database_path.parent.is_dir()
